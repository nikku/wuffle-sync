...
function findLinks(issue, types) {
  ...
  const repositoryData = getRepositoryData(issue);
  ...
  while ((match = pattern.exec(text))) {
    ...
    const owner = match[2] || match[5] || repositoryData.owner;
    const repo = match[3] || match[6] || repositoryData.repo;
...
function getRepositoryData(issue) {
