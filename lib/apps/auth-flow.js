...
module.exports = async (app, config, store) => {
  ...
  const log = app.log.child({ name: 'auth-flow' });
  ...
  app.router.get('/wuffle/login_check', withSession, cors(async (req, res) => {
    ...
    try {
      ...
      log.info({ session_id: session.id }, 'fetched GitHub profile');
      ...
      log.warn({ session_id: session.id }, 'failed to retrieve GitHub profile', error);
